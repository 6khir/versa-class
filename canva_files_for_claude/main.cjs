...
const { FileManager, collectProductPageImagePaths, prepareCanvaImportPdf } = require('./file-manager.cjs');
const { FORMAT_INSTRUCTIONS, buildBookJobs, buildImportedJobs, buildStorybookJobs, cleanText, normalizeOrientation, parseAnalysisResponse, parseGeneratedPrompts, parseTptListingResponse, slugify } = require('./prompt-builder.cjs');
const {
  normalizeEngine,
  engineDisplayName,
  getJobStartUrl,
...
function canvaStepFromProgress(message = '', percent = 0) {
  ...
  if (/checking layer|audit/.test(text) || value >= 82) return 4;
  if (/page \d+|applying magic layer|magic layer applied|not separated/.test(text) || value >= 20) return 3;
  if (/import|print pdf|imported/.test(text) || value >= 8) return 2;
  return 1;
}
...
function mergeCanvaPageProgress(list, pageNumber, patch) {
  ...
  else rows.push({ pageNumber: Number(pageNumber), uploaded: false, layered: false, error: null, ...patch });
...
async function ensureProductPdf(projectId, { force = false } = {}) {
  const project = store.getProject(projectId);
  if (!project) return null;
  if (project.stats.complete !== project.stats.total || !project.stats.total) return null;
  const existing = project.productPdfPath || project.tptListing?.productPdfPath;
  if (!force && existing && existsSync(existing)) {
    if (existing !== project.productPdfPath) {
      store.updateProject(projectId, { productPdfPath: existing });
    }
    return existing;
  }
  const pdfPath = await fileManager.exportPdf(project);
  const listing = project.tptListing ? { ...project.tptListing, productPdfPath: pdfPath } : project.tptListing;
  ...
  store.appendEvent({
    projectId,
    level: 'success',
    message: `Book PDF ready (${project.format} ${project.orientation}). Canva can upload this file next.`
  });
  return pdfPath;
}
...
async function runCanvaEditableForProject(projectId, onProgress = () => {}) {
  ...
  const importPdf = await prepareCanvaImportPdf(
    pdfPath,
    project.outputDir,
    project.format,
    project.orientation,
  ...
  store.updateProject(projectId, { canvaPageProgress, stepEditableStatus: 'processing' });
  await onProgress(4);
  store.appendEvent({ projectId, message: 'Preparing the print PDF for Canva import…' });
  await publishLiveOperation({
    ...
    stepIndex: 1,
    stepCount: 6,
    message: 'Preparing the print PDF for Canva import…',
    projectId,
    canvaPageProgress
  });
  let result;
  try {
    result = await browser.runCanvaBulkCreate({
      ...
      downloadDir: join(project.outputDir, 'canva-export'),
      onProgress: async (info) => {
        ...
        setLiveOperation({
          ...
          stepIndex: canvaStepFromProgress(message, percent),
          stepCount: 6,
          message,
          projectId,
        ...
        if (info?.designUrl) {
          store.updateProject(projectId, {
            canvaDesignUrl: toCanvaDesignUrl(info.designUrl) || info.designUrl
          });
        }
        if (info?.message && !info?.heartbeat && !/^Checking layer count on page \d+ of \d+/i.test(info.message) && !/^Still (importing|applying|on Canva)|Waiting for Magic Layer to start|Canva opened a design/i.test(info.message)) {
          store.appendEvent({ projectId, message: info.message });
        }
        await broadcastState();
      }
    });
  ...
  store.appendEvent({
    ...
    message: missedPages.length
      ...
      : result.resumed
        ? `Canva design continued from the imported PDF. Template link saved.`
...
async function openStudioById(studioId = 'planning') {
  ...
  try {
    ...
    await browser.launch({ forceBrowser: true });
...
function assertIdle(action = 'starting this workflow') {
  ...
  if (automation?.getStatus().active) {
...
function registerIpc() {
  ...
  ipcMain.handle('analysis:generate-prompts', async (_event, payload = {}) => {
    ...
    if (projectId) {
      ...
      const result = await browser.generatePromptsWithGpt({
        ...
        seed: `${existingProject.id} | ${existingProject.name} | ${existingProject.theme} | ${existingProject.niche}`
      });
  ...
  ipcMain.handle('project:run-canva-editable', async (_event, projectId) => {
    assertIdle();
  ...
  ipcMain.handle('project:clear-canva-template', async (_event, projectId) => {
    ...
    store.appendEvent({ projectId, message: 'Canva template link removed. You can rebuild a new Magic Layer file.' });
    await broadcastState();
    return store.getProject(projectId);
  });
  ...
  ipcMain.handle('browser:focus', () => browser.bringToFront());
  ...
  ipcMain.handle('project:generate-tpt-listing', async (_event, projectId) => {
    assertIdle();
    requireListingEngine('preparing a listing');
    const project = store.getProject(projectId);
    if (!project) throw Object.assign(new Error('Project not found.'), { code: 'PROJECT_NOT_FOUND' });
    if (project.stats.complete !== project.stats.total || project.stats.total === 0) {
      throw Object.assign(new Error('Complete every page before preparing a listing.'), { code: 'BOOK_INCOMPLETE' });
    }
    try {
      await publishLiveOperation({
        kind: 'listing',
        label: 'TPT listing',
        percent: 8,
        message: 'Using the print-size book PDF for the listing…',
        projectId
      });
      const pdfPath = await ensureProductPdf(projectId);
      if (!pdfPath) {
        throw Object.assign(new Error('Complete every page before preparing a listing. The book PDF is built automatically when pages finish.'), {
      ...
      await publishLiveOperation({
        ...
        label: 'TPT listing',
        percent: 32,
        message: 'Writing title, description, tags, and grades…',
        projectId
      });
      ...
      await publishLiveOperation({
        ...
        label: 'TPT listing',
        percent: 88,
        message: 'Saving the listing draft into this book…',
        projectId
      });
      const listing = {
      ...
      store.appendEvent({ projectId, level: 'success', message: 'GPT listing draft generated from the finished PDF.' });
      return store.getProject(projectId);
    } catch (error) {
      if (isPauseError(error)) {
  ...
  ipcMain.handle('project:generate-tpt-thumbnails', async (_event, projectId) => {
    ...
    const project = store.getProject(projectId);
    const listing = project?.tptListing;
    if (!project || !listing?.productPdfPath) throw Object.assign(new Error('Create the GPT listing draft first.'), { code: 'TPT_LISTING_REQUIRED' });
    const thumbnailPaths = [...(listing.thumbnailPaths ?? [])];
    ...
    try {
      ...
      store.appendEvent({ projectId, level: 'success', message: 'Four GPT listing thumbnails generated from the product PDF.' });
      return store.getProject(projectId);
    } catch (error) {
      if (isPauseError(error)) {
  ...
  ipcMain.handle('project:update-tpt-publication', async (_event, projectId, settings = {}) => {
    ...
    const isFreeResource = settings.isFreeResource === true;
    const suggestedPrice = cleanText(settings.suggestedPrice);
    const multipleLicensePrice = cleanText(settings.multipleLicensePrice);
    const bundleDiscountPrice = cleanText(settings.bundleDiscountPrice);
    const copyrightDeclaration = ['original', 'licensed'].includes(settings.copyrightDeclaration) ? settings.copyrightDeclaration : '';
    const compactList = (value, limit) => String(value ?? '').split(/[\n,]/).map((item) => cleanText(item)).filter(Boolean).slice(0, limit);
    const title = cleanText(settings.title, listing.title || project.name);
    const description = cleanText(settings.description, listing.description || '');
    const taxCode = cleanText(settings.taxCode);
    ...
    const publicationStatus = settings.publicationStatus === 'active' ? 'active' : 'draft';
    const thumbnailMode = ['auto', 'manual', 'later'].includes(settings.thumbnailMode) ? settings.thumbnailMode : (listing.thumbnailMode || 'manual');
...
if (singleInstanceAcquired) app.whenReady().then(() => {
  ...
  protocol.handle('tpt-image', async (request) => {
    ...
    try {
      ...
      if (url.hostname === 'character') {
        ...
        return net.fetch(pathToFileURL(sheet.outputPath).toString());
      }
      ...
      if (url.hostname === 'competitor-mockup') {
        ...
        return net.fetch(pathToFileURL(mockupPath).toString());
      }
      if (url.hostname !== 'job') {
        return new Response('Not found', { status: 404 });
      }
  ...
  automation = new AutomationManager({
    ...
    stepVerifiers: {
      ...
      interior: (projectId) => {
        const stats = store.getProject(projectId)?.stats;
      ...
      editable: (projectId) => {
        ...
        if (!isCanvaTemplateLink(link)) {
          throw Object.assign(
            new Error('The Canva editable template link was not saved.'),
            { code: 'CANVA_TEMPLATE_LINK_MISSING' }
          );
        }
      }
    ...
    stepRunners: {
      ...
      // interior: start the queue and wait for all pages to complete
      ...
      // listing: generate GPT listing via SEO Gem
      listing: async (projectId, onProgress) => {
        requireListingEngine('listing generation');
        const project = ensureProjectOutputDirectory(projectId);
        if (!project) throw Object.assign(new Error('Project not found.'), { code: 'PROJECT_NOT_FOUND' });
        const pdfPath = await ensureProductPdf(projectId);
        if (!pdfPath) {
        ...
        store.updateProject(projectId, { tptListing: listing });
        store.appendEvent({ projectId, level: 'success', message: '[Automation] GPT listing generated.' });
        onProgress(100);
        await broadcastState();
      },
      ...
      thumbnails: async (projectId, onProgress) => {
        ...
        if (!project || !listing?.productPdfPath) {
          throw Object.assign(new Error('Create the GPT listing first.'), { code: 'TPT_LISTING_REQUIRED' });
        }
        const thumbnailPaths = [...(listing.thumbnailPaths ?? [])];
        store.updateProject(projectId, { tptListing: invalidateTptListingReview(listing, { thumbnailPaths, status: 'thumbnails_generating', thumbnailProgress: { completed: thumbnailPaths.length, total: 4 } }) });
        await broadcastState();
      ...
      export: async (projectId, onProgress) => {
        ...
        await fileManager.exportPdf(project);
        onProgress(50);
