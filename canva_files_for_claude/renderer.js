...
function setStagePresentation(stageId, state, extraClass = {}) {
  ...
  if (!card) return;
  const states = ['done', 'live', 'progress', 'waiting', 'blocked', 'skipped'];
...
async function invoke(action, { successMessage = null, refresh = true } = {}) {
  try {
    ...
    const info = ['QUEUE_BUSY', 'QUEUE_RUNNING', 'AUTOMATION_BUSY', 'QUEUE_PAUSED'].includes(code)
...
function renderSettingsConnections() {
  ...
  const canvaConnected = Boolean(canva.connected);
  ...
  if (elements.settingsCanvaCard) {
    ...
    if (elements.settingsCanvaStatus) {
      elements.settingsCanvaStatus.textContent = canvaConnected ? 'Verified Canva Pro connection' : 'Connection required';
    }
    ...
    if (elements.settingsCanvaProfile) {
      ...
      elements.settingsCanvaProfile.textContent = canvaConnected
    ...
    if (elements.settingsCanvaVerified) {
      ...
      elements.settingsCanvaVerified.textContent = canvaVerifiedParts.join(' · ');
    }
  }
...
function renderCanvaProgress(project) {
  ...
  document.querySelectorAll('[data-canva-step]').forEach((item) => {
    ...
    const finished = done || (activeStep && step < activeStep);
    const current = Boolean(canvaOp) && step === activeStep;
    item.classList.toggle('is-done', finished);
    item.classList.toggle('is-active', current);
    item.classList.toggle('is-waiting', !finished && !current);
    const fill = item.querySelector('.step-fill');
    const pct = item.querySelector('.step-percent');
    const hint = item.querySelector('small');
    const width = finished ? 100 : current ? Math.max(18, percent) : 8;
    if (fill) fill.style.width = `${width}%`;
    if (pct) pct.textContent = `${finished ? 100 : current ? Math.round(percent) : 0}%`;
    if (hint) hint.textContent = finished ? 'Finished' : current ? (canvaOp.message || 'In progress') : 'Waiting';
  });
  ...
  if (elements.canvaProgressMessage) {
    elements.canvaProgressMessage.textContent = canvaOp?.message
      || (done ? 'Saved.' : 'Idle. Empty page slots fill in as Magic Layer is applied.');
  }
  if (elements.canvaProgressLabel) {
    elements.canvaProgressLabel.textContent = canvaOp ? 'Live' : done ? 'Ready' : 'Canva';
  }
  ...
  document.getElementById('canva-live-bar')?.classList.toggle('is-live', Boolean(canvaOp));
  if (elements.clearCanvaTemplateButton) {
    elements.clearCanvaTemplateButton.disabled = !project?.canvaTemplateLink && !project?.canvaDesignUrl;
  }
...
function tptListingHtml(project, view = activeWorkspaceView) {
  ...
  return `<div class="section-header tpt-section-heading"><div><p class="eyebrow">TPT</p><h3>${showListing ? 'Listing' : 'Mockups'}</h3></div>
      <div class="export-actions">
        ${showListing ? `<button class="button button-primary" data-action="generate-tpt-listing" type="button">${hasListing ? 'Regenerate listing' : 'Generate listing'}</button>
        <button class="button button-ghost button-danger" data-action="clear-tpt-listing" type="button" ${hasListing ? '' : 'disabled'}>Delete listing</button>` : `<button class="button button-primary" data-action="generate-tpt-thumbnails" type="button" ${hasListingDraft ? '' : 'disabled'}>Generate mockups</button>
...
function renderProject() {
  ...
  const stageLive = {
    overall: Boolean(liveOp || queueHere),
    characters: false,
    interior: queueHere,
  ...
  Object.entries(stagePercent).forEach(([id, pct]) => {
    ...
    const stateName = skipped ? 'skipped' : live ? 'live' : pct >= 100 ? 'done' : blocked ? 'blocked' : pct > 0 ? 'progress' : 'waiting';
  ...
  if (elements.overviewEditableStatus) {
    const editable = project.productFormat === 'editable';
    elements.overviewEditableStatus.textContent = !editable
      ? 'Static print'
      : project.canvaTemplateLink
        ...
        : stats.complete === stats.total && stats.total > 0
          ? 'Ready to build'
          : 'Waiting for pages';
    elements.overviewEditableDetail.textContent = !editable
      ? 'Toggle Editable'
      : project.canvaTemplateLink
        ? 'Template saved'
        : 'After pages finish';
  ...
  if (elements.canvaEditableStatus) {
    const editable = project.productFormat === 'editable';
    elements.canvaEditableStatus.textContent = !editable
  ...
  if (elements.runCanvaEditableButton) {
    elements.runCanvaEditableButton.disabled = project.productFormat !== 'editable' || stats.complete !== stats.total || stats.total === 0;
  }
  ...
  elements.runButton.disabled = isPipelineBusy() || stats.remaining === 0;
  ...
  if (elements.generateTptListingButton) {
    elements.generateTptListingButton.disabled = !canExport || state.queue.running;
  }
  if (elements.generateTptThumbnailsButton) {
    elements.generateTptThumbnailsButton.disabled = !project.tptListing?.productPdfPath || state.queue.running;
  }
  if (elements.generateTptPreviewVideoButton) {
    elements.generateTptPreviewVideoButton.disabled = !listingReady || thumbnailCount === 0 || state.queue.running;
  }
...
async function handleAction(action, target) {
  ...
  if (action === 'toggle-product-format') {
    ...
    return invoke(() => api.setProjectFormat(project.id, next), {
      successMessage: next === 'editable' ? 'Marked editable (Canva layer).' : 'Marked static (print only).'
    });
  }
...
elements.generateTptListingButton?.addEventListener('click', () => handleAction('generate-tpt-listing', elements.generateTptListingButton).catch(() => {}));
elements.generateTptThumbnailsButton?.addEventListener('click', () => handleAction('generate-tpt-thumbnails', elements.generateTptThumbnailsButton).catch(() => {}));
if (elements.generateTptPreviewVideoButton) {
  elements.generateTptPreviewVideoButton.addEventListener('click', () => handleAction('generate-tpt-preview-video', elements.generateTptPreviewVideoButton).catch(() => {}));
}
elements.openTptUploadButton?.addEventListener('click', () => handleAction('open-tpt-upload', elements.openTptUploadButton).catch(() => {}));
