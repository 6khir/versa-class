...
const { selectPreviewAttachmentPaths } = require('./file-manager.cjs');
const {
  CANVA_HOME_URL,
  canvaDesignId,
  canvaPageDimensions,
...
function collectChatGptImageCandidatesInBrowser() {
  const assistantSel = '[data-message-author-role="assistant"], [data-turn="assistant"]';
  ...
  for (const selector of selectors) {
    for (const image of document.querySelectorAll(selector)) {
      ...
      results.push({
        ...
        slot,
        generatedHint: /blob:|data:image|oaiusercontent|generated image|image-gen/i.test(`${raw} ${slot} ${image.getAttribute('alt') || ''}`),
        ...
        width: image.naturalWidth || Math.round(image.getBoundingClientRect().width) || 0,
        height: image.naturalHeight || Math.round(image.getBoundingClientRect().height) || 0
      });
    }
  }
  return results;
...
function collectGeminiImageCandidatesInBrowser() {
  const assistantSel = 'model-response, response-container, chat-app message-content, .message-content';
  ...
  const consider = (image) => {
    ...
    results.push({
      ...
      alt: image.getAttribute('alt') || '',
      slot: '',
      generatedHint: /blob:|data:image|googleusercontent|generated image/i.test(`${raw} ${image.getAttribute('alt') || ''}`)
        || Boolean(image.closest('generated-image, model-response, response-container')),
      fromUserTurn: false,
      inComposer: false,
      assistantIndex: assistantMessage ? assistantMessages.indexOf(assistantMessage) : -1,
      width,
...
function collectMetaImageCandidatesInBrowser() {
  const seen = new Set();
  const results = [];
  const preferred = [...document.querySelectorAll('div[data-pressable-container="true"] img, img[alt*="Meta AI" i]')];
  ...
  for (const image of [...preferred, ...rest]) {
    ...
    if (seen.has(signature)) continue;
    seen.add(signature);
    const inMetaSurface = Boolean(image.closest('div[data-pressable-container="true"]') || /meta ai/i.test(image.getAttribute('alt') || ''));
    results.push({
      signature,
      src: raw,
      alt: image.getAttribute('alt') || '',
      slot: '',
      generatedHint: inMetaSurface || /blob:|data:image|scontent|fbcdn|cdninstagram|meta\.ai|facebook/i.test(raw),
...
async function extractRenderedImageInBrowser(src) {
  ...
  const surfaceSel = '[data-testid="image-gen-card"], [data-testid*="image-gen"], div[id^="image-"], generated-image, model-response, div[data-pressable-container="true"]';
  const imgs = [...document.querySelectorAll('img')];
  let image = imgs.find((element) => element.src === wanted || element.currentSrc === wanted);
  if (!image) {
    image = document.querySelector(`${surfaceSel} img, img[alt*="Generated image" i], img[alt*="Meta AI" i], generated-image img, model-response img, response-container img, .message-content img`);
  }
...
function mergeTptCookies(targetCookiePath, preservedCookiePath) {
  if (!existsSync(targetCookiePath) || !existsSync(preservedCookiePath)) return 0;
  let source = null;
  let target = null;
  try {
...
class BrowserController extends EventEmitter {
  ...
  async importSystemLoginSession(selectedProfile = null) {
    ...
    try {
      ...
      preservedTptCookieCount = mergeTptCookies(targetCookiePath, preservedTptCookiePath);
      }
  ...
  async #ensureLiveGeminiStudio(page, targetUrl) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (!deleted && !wrongGem) return page;
  ...
  async #findOrCreateChatPage() {
    ...
    if (!page) {
      page = pages.find((item) => this.engine === 'gemini' && isGeminiPageUrl(item.url()))
        || await this.#ensureBackgroundPage();
    }
  ...
  async #jobPage(jobId, { fresh = false, url = null } = {}) {
    ...
    if (jobPageNeedsNavigation(page.url(), targetUrl, fresh) || isRetiredGeminiGemUrl(page.url())) {
  ...
  async detectBlocker(page = null) {
    ...
    const geminiAuth = isGeminiPageUrl(url) && (/accounts\.google\.com/i.test(url) || authMessageVisible || await this.#hasVisibleLoginControl(page));
  ...
  async authenticationStatus(page = null, { forceTarget = null } = {}) {
    ...
    if (rawTarget === 'canva' || isCanvaPageUrl(page?.url?.() ?? '')) {
  ...
  async assertAuthenticated(page = null) {
    const status = await this.authenticationStatus(page);
  ...
  async #verifyLoginOnce(selectedProfile, { timeoutMs, pollIntervalMs, target }) {
    ...
    if (!this.context && !alreadySaved) {
      try {
        imported = await this.importSystemLoginSession(selectedProfile);
      } catch (caughtError) {
  ...
  async submitPrompt(prompt, {
    ...
    const startUrl = conversationUrl || gptUrl || getJobStartUrl({
      kind: promptKind === 'preview-video' ? 'preview' : 'page',
      purpose: 'image'
    }, this.engine);
    let page;
    if (conversationUrl) {
  ...
  async waitForNewImage(baseline = [], timeoutMs = 600_000, { jobId = null, idleTimeoutMs = null } = {}) {
    const page = jobId ? await this.#jobPage(jobId) : await this.ensurePage();
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      if (candidate) {
        ...
        if (generationInProgress) {
          ...
          if (!(candidate.width >= 512 && Date.now() - pendingSince >= 8_000)) {
            await this.#sleepOrPause(1_000);
            continue;
          }
        }
        ...
        if (this.#pageKind(page) === 'meta' && isNewAssistantImage(stable, known, assistantBaselineCount)) {
          ...
          for (let poll = 0; poll < 4; poll += 1) {
            ...
            if (stillBusy || next.src !== previousSrc) {
  ...
  async scrapeTptListingMockups(productUrl, destDir) {
    ...
    const page = await this.#ensureBackgroundPage();
    try {
      ...
      console.log(`[browser] TPT listing scrape ${parsed.href} extra=${Array.isArray(extraUrls) ? extraUrls.length : 0} urls=${urls.length} title=${pageTitle}`);
      if (!urls.length) {
        return emptyMockupResult({
          status: 'empty',
          productUrl: parsed.href,
      ...
      if (page && page !== this.page) await page.close().catch(() => {});
      else if (page) await this.#afterNavigate(page);
    }
  }
  ...
  async analyzeProductWithGpt(input = {}) {
    ...
    const prompt = buildAnalysisPrompt({ ...listing, mockupCount: attachmentPaths.length });
  ...
  async generatePromptsWithGpt({
    ...
    const prompt = buildPromptsGenerationRequest(pageCount, format, orientation, {
      ...
      seed: seed || [title, theme, niche].filter(Boolean).join(' | '),
      title,
      theme,
      niche,
      visualTheme
  ...
  async generateTptListingWithGpt({ project, pdfPath, gptUrl = null }) {
    ...
    try {
      let submission = await this.submitPrompt(prompt, { jobId, isolatedPage: true, attachmentPath: pdfPath, gptUrl });
      conversationUrl = submission.conversationUrl;
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        submission = await this.submitPrompt(correctionPrompt, { jobId, conversationUrl });
        conversationUrl = submission.conversationUrl;
  ...
  async generateTptThumbnailsWithGpt({ project, pdfPath, listing, onThumbnail = null, thumbnailIndex = null, gptUrl = null }) {
    ...
    for (let index = 0; index < 4; index += 1) {
      ...
      const prompt = withEngineImagePrefix([
        `Create thumbnail ${index + 1} of 4 for this Teachers Pay Teachers product.`,
        `Product title: ${listing.title || project.name}.`,
        `Creative direction: ${brief}`,
        'Create a polished 2000x2000 square product-listing graphic, bold readable headline, uncluttered layout, no watermark, and no invented claims. This is a thumbnail, not a printable worksheet.'
      ].join('\n'), this.engine);

      let attachments = [];
      if (project && project.jobs && project.jobs.length > 0) {
  ...
  async #canvaPage() {
    if (this.canvaWatch) this.interactiveVisible = true;
    await this.launch({ skipHome: true, forceBrowser: true, interactive: this.interactiveVisible });
    if (!this.context) {
      throw Object.assign(new Error('A Google Chrome Canary browser context is not active.'), { code: 'BROWSER_NOT_LAUNCHED' });
    }
    if (this.canvaPage && !this.canvaPage.isClosed()) {
      if (canvaDesignId(this.canvaPage.url())) {
        if (this.canvaWatch) await this.#revealCanvaBrowser(this.canvaPage);
        return this.canvaPage;
      }
    }
    const pages = this.context.pages().filter((item) => item && !item.isClosed());
    let page = pages.find((item) => canvaDesignId(item.url()))
      || pages.find((item) => isCanvaPageUrl(item.url()));
    if (!page) {
      page = pages.find((item) => item.url() === 'about:blank' && !item.isClosed()) || await this.#ensureBackgroundPage();
    }
    this.canvaPage = page;
    if (this.canvaWatch) await this.#revealCanvaBrowser(page);
    else await this.#afterNavigate(page);
    return page;
  }
    ...
    while (Date.now() - startedAt < budget) {
      ...
      for (const role of roles) {
        ...
        if (await locator.isVisible({ timeout: 0 }).catch(() => false)) {
          await locator.click({ timeout: Math.min(1_200, budget) }).catch(() => {});
          return true;
        }
      }
      const text = page.getByText(pattern).first();
      if (await text.isVisible({ timeout: 0 }).catch(() => false)) {
        await text.click({ timeout: Math.min(1_200, budget) }).catch(() => {});
        return true;
      }
  ...
  async #waitForCanvaEditor(page, timeoutMs = 90_000) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeoutMs) {
      this.#throwIfCancelled();
  ...
  async #canvaImageToolbarVisible(page) {
    return page.evaluate(() => {
      ...
      return blocks.some((el) => {
        ...
        const hasPhotoTools = /bg remover|background remover|eraser|flip|effects|animate|position/i.test(text);
  ...
  async #canvaEditImagePanelOpen(page) {
    const heading = await page.getByText(/^edit image$/i).first().isVisible({ timeout: 0 }).catch(() => false);
    ...
    return Boolean(heading && magic);
  }

  async #canvaMagicLayersBusy(page) {
    const busyPattern = /processing|turning .+ into|applying magic|magic layers is working|creating layers|extracting|separating|preparing your layers|working on it|just a (sec|moment|second)|hang tight|this may take/i;
    return page.getByText(busyPattern).first().isVisible({ timeout: 0 }).catch(() => false);
  }

  async #canvaCanvasClickPoint(page) {
    const point = await page.evaluate(() => {
      ...
      const rects = [...document.querySelectorAll('canvas, [data-testid*="canvas" i]')]
        .map((el) => el.getBoundingClientRect())
        .filter((box) => box.width > 180 && box.height > 180 && box.left < maxRight && box.right > 90);
      rects.sort((left, right) => (right.width * right.height) - (left.width * left.height));
      const box = rects[0];
      if (!box) {
        return { x: Math.min(window.innerWidth * 0.36, maxRight - 80), y: window.innerHeight * 0.48 };
      }
      return { x: box.left + (box.width * 0.5), y: box.top + (box.height * 0.48) };
    }).catch(() => null);
    if (point && Number.isFinite(point.x) && Number.isFinite(point.y)) return point;
    return { x: 460, y: 420 };
  }

  async #canvaSelectPageImage(page) {
    await this.#canvaDismissPrintReview(page);
    if (await this.#canvaImageToolbarVisible(page)) return true;
    const point = await this.#canvaCanvasClickPoint(page);
    const offsets = [[0, 0], [0, -22], [-28, 14], [22, 18]];
    for (const [dx, dy] of offsets) {
      this.#throwIfCancelled();
      await page.mouse.click(point.x + dx, point.y + dy).catch(() => {});
      await this.#sleepOrPause(90);
      if (await this.#canvaImageToolbarVisible(page)) return true;
    }
    await page.mouse.dblclick(point.x, point.y).catch(() => {});
  ...
  async #canvaClickToolbarEdit(page) {
    const clicked = await page.evaluate(() => {
      ...
      const hit = nodes.find((el) => {
        ...
        const label = `${el.getAttribute('aria-label') || ''} ${el.textContent || ''}`.replace(/\s+/g, ' ').trim();
        if (!/^(edit|edit image)$/i.test(label)) return false;
        const bar = el.closest('[role="toolbar"]') || el.parentElement || el;
        const nearby = `${bar.getAttribute('aria-label') || ''} ${bar.innerText || ''}`.replace(/\s+/g, ' ');
        return /bg remover|background remover|eraser|flip|effects|animate|position/i.test(nearby);
      });
      if (!hit) return false;
      hit.click();
      return true;
    ...
    if (clicked) return true;
    return this.#canvaClickExact(page, /^(edit|edit image)$/, 1_200);
  }

  async #canvaClickMagicLayersTool(page) {
    return this.#canvaClickExact(page, /^(magic layers|magic layer)\b/, 900)
      || this.#canvaClickFirst(page, [/^magic layers$/i, /^magic layer$/i], 400);
  }
  ...
  async #canvaWaitWhileMagicLayersRuns(page, timeoutMs = 150_000, onTick = null) {
    const startedAt = Date.now();
    const spinDeadline = startedAt + 12_000;
    const busyPattern = /processing|turning .+ into|applying magic|magic layers is working|creating layers|extracting|separating|preparing your layers|working on it|just a (sec|moment|second)|hang tight|this may take/i;
    ...
    await tick();
    if ((await this.#canvaInspectLayerCount(page)) > 1) return true;
    while (Date.now() - startedAt < timeoutMs) {
      this.#throwIfCancelled();
      const blocked = await page.getByText(/upgrade|not available|can't use magic layers|couldn['’]t (apply|process)|try again later/i).first().isVisible({ timeout: 0 }).catch(() => false);
      if (blocked) {
      ...
      const busyText = await this.#canvaMagicLayersBusy(page);
      const busyBar = await page.locator('[role="progressbar"]').first().isVisible({ timeout: 0 }).catch(() => false);
      const busy = Boolean(busyText || (sawBusy && busyBar));
      const layered = (await this.#canvaInspectLayerCount(page)) > 1;
      if (layered) return true;
      if (busy) {
        sawBusy = true;
      } else if (sawBusy) {
        return true;
      } else if (Date.now() >= spinDeadline) {
    ...
    const stillBusy = await page.getByText(busyPattern).first().isVisible({ timeout: 0 }).catch(() => false);
    if (stillBusy) {
  ...
  async #canvaClickCreateLayers(page) {
    return await this.#canvaClickExact(page, /^(create layers|apply magic layers)$/, 280)
      || await this.#canvaClickLabeled(page, /^(create layers|apply magic layers)$/);
  }

  async #canvaOpenLayersPanel(page) {
    await this.#canvaClickFirst(page, [/^position$/i, /^layers$/i, /show layers/i, /positioning/i], 500);
  }
  ...
  async #canvaInspectLayerCount(page) {
    const result = await page.evaluate(() => {
      ...
      if (treeItems.length >= 1) return treeItems.length;
  ...
  async #canvaOpenMagicLayersInEditor(page, onTick = null) {
    await this.#canvaDismissPrintReview(page);
    await this.#canvaDismissTours(page);
    if (await this.#canvaMagicLayersBusy(page)) {
      await this.#canvaWaitWhileMagicLayersRuns(page, 180_000, onTick);
      return;
    }
    ...
    const clickedMagic = await this.#canvaClickMagicLayersTool(page);
    if (!clickedMagic) {
    ...
    if (await this.#canvaMagicLayersBusy(page) || (await this.#canvaInspectLayerCount(page)) > 1) {
      await this.#canvaWaitWhileMagicLayersRuns(page, 180_000, onTick);
      return;
    }
    await this.#canvaClickCreateLayers(page);
    await this.#canvaWaitWhileMagicLayersRuns(page, 180_000, onTick);
  }
  ...
  async #canvaSelectEditorPage(page, pageNumber) {
    ...
    for (const thumb of candidates) {
      if (!(await thumb.isVisible({ timeout: 0 }).catch(() => false))) continue;
      await thumb.click({ timeout: 1_200 }).catch(() => {});
      await this.#sleepOrPause(80);
  ...
  async #canvaEditorPageCount(page) {
    return page.evaluate(() => {
      const ids = new Set();
      let statedTotal = 0;
      const consider = (raw) => {
        ...
        if (!text) return;
        const ofMatch = text.match(/\bpage\s*(\d+)\s*(?:of|\/)\s*(\d+)\b/i);
        if (ofMatch) {
          ids.add(ofMatch[1]);
          statedTotal = Math.max(statedTotal, Number(ofMatch[2]));
        }
        const pageMatch = text.match(/\bpage\s*(\d+)\b/i);
        if (pageMatch) ids.add(pageMatch[1]);
        const pagesMatch = text.match(/\b(\d{1,3})\s+pages?\b/i);
        if (pagesMatch) statedTotal = Math.max(statedTotal, Number(pagesMatch[1]));
      };
      for (const node of document.querySelectorAll('[aria-label], [title], [aria-roledescription], button, [role="button"], [role="listitem"], [data-page-index], [data-testid*="page" i], [data-test-id*="page" i]')) {
        consider(`${node.getAttribute('aria-label') || ''} ${node.getAttribute('title') || ''} ${node.textContent || ''}`);
        const index = node.getAttribute('data-page-index') || node.getAttribute('data-index');
        if (index != null && /^\d+$/.test(index)) ids.add(String(Number(index) + 1));
      }
      return Math.max(ids.size, statedTotal);
    }).catch(() => 0);
  }
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      try {
        ...
        if (!(await this.#canvaSelectEditorPage(page, pageNumber))) {
          throw Object.assign(new Error(`Canva did not select page ${pageNumber} of ${expectedPages} in the page panel.`), {
            code: 'CANVA_PAGE_THUMBNAIL_MISSING'
          });
        }
        await this.#canvaDismissPrintReview(page);
  ...
  async #setAnyPageFileInput(page, files) {
    const locators = [];
    try {
      locators.push(page.locator('input[type="file"]'));
    } catch {}
    try {
      for (const frame of page.frames()) {
        locators.push(frame.locator('input[type="file"]'));
      }
    } catch {}
    for (const locator of locators) {
      ...
      for (let index = count - 1; index >= 0; index -= 1) {
        try {
          await this.#setDiskFilesOnPage(page, files, { locator: locator.nth(index) });
          return true;
        } catch {}
      }
    }
    return this.#cdpSetAnyFileInput(page, files);
  }
    ...
    try {
      ...
      const evaluated = await session.send('Runtime.evaluate', {
        expression: `(() => {
          const visit = (root) => {
            const found = [];
            try { found.push(...root.querySelectorAll('input[type="file"]')); } catch {}
            try {
              for (const el of root.querySelectorAll('*')) {
                if (el.shadowRoot) found.push(...visit(el.shadowRoot));
              }
            } catch {}
            return found;
          };
  ...
  async #pickLocalFilesInPage(page, paths, { clickName = /upload files|^upload$/i, mediaPicker = false } = {}) {
    this.#throwIfCancelled();
    const files = localUploadFiles(paths);
    if (!files.length) {
      throw Object.assign(new Error('No local files were available to upload.'), { code: 'UPLOAD_FILES_MISSING' });
    ...
    let chooser = null;
    const chooserWait = this.#waitForFileChooserOrAbort(page, 4_000).then((value) => {
      chooser = value;
      return value;
    });
    ...
    const uploadTook = async (waitMs = 1_800) => {
      const until = Date.now() + waitMs;
      while (Date.now() < until) {
        if (await this.#canvaUploadTook(page)) return true;
        await this.#sleepOrPause(60);
      }
      return this.#canvaUploadTook(page);
    };
    const tryAccept = async () => (await acceptFiles()) && (await uploadTook());

    if (await tryAccept()) return;

    const clickPatterns = mediaPicker
      ? [
        /import files/i,
        /import file/i,
        /select media/i,
    ...
    await this.#canvaClickByName(page, clickName || /select media/i, 500);
    await this.#sleepOrPause(80);
    if (await tryAccept()) return;
    ...
    this.#throwIfCancelled();
    if (chooser && await this.#applyChooserFiles(page, chooser, files) && await uploadTook()) return;
    if (await this.#setAnyPageFileInput(page, files) && await uploadTook()) return;
    if (await this.#canvaDropLocalFiles(page, files) && await uploadTook(2_400)) return;

    throw Object.assign(new Error('Canva did not take the print PDF. Stay on Create a design → Import file, then try again.'), {
  ...
  async #canvaAdoptDesignPage(preferred = null) {
  ...
  async #canvaUploadTook(page) {
    if (canvaDesignId(page?.url?.() || '')) return true;
    if (await this.#canvaImportIndicatorVisible(page)) return true;
    const adopted = await this.#canvaAdoptDesignPage(page);
    return Boolean(adopted && canvaDesignId(adopted.url()));
  }
  ...
  async #canvaTakeImportTarget(page, popupPromise) {
    ...
    return await this.#canvaAdoptDesignPage(page) || page;
  }
  ...
  async #canvaImportIndicatorVisible(page) {
    const pattern = /importing|uploading (your )?(file|pdf|document)|converting|processing your (file|pdf|document)|preparing your (design|file)|this may take a (few )?(minutes|moments)/i;
    if (await page.getByText(pattern).first().isVisible({ timeout: 0 }).catch(() => false)) return true;
    const bar = page.locator('[aria-label*="importing" i], [aria-label*="uploading" i], [aria-label*="import" i][role="progressbar"]').first();
    return bar.isVisible({ timeout: 0 }).catch(() => false);
  }

  async #canvaWaitForPdfImport(page, { expectedPages, timeoutMs = 4 * 60_000, report = null, percent = 12 } = {}) {
    const startedAt = Date.now();
    let lastTickAt = 0;
    let lastNudgeAt = 0;
    const tick = async (count, importing, href) => {
      if (typeof report !== 'function') return;
      const now = Date.now();
      if (now - lastTickAt < 4_000 && lastTickAt) return;
      lastTickAt = now;
      const elapsed = formatCanvaClock(now - startedAt);
      const seen = Number(count) || 0;
      const inEditor = Boolean(canvaDesignId(href));
      let message;
      if (!inEditor && seen === 0) {
        message = `Still on Canva home (${elapsed}). The print PDF has not opened as a design yet — 0 of ${expectedPages} pages. Looking for Import file / Edit this PDF.`;
      } else if (inEditor && seen === 0) {
        message = `Canva opened a design (${elapsed}) but no page thumbnails yet. Waiting for the PDF to expand to ${expectedPages} pages.`;
      } else {
        message = `Still importing the print PDF (${elapsed}). Canva has ${seen} of ${expectedPages} pages${importing ? ' and is still converting' : ''}.`;
      }
      await report(percent, message, { heartbeat: true });
    };
    await tick(0, true, page.url());
    while (Date.now() - startedAt < timeoutMs) {
      this.#throwIfCancelled();
      page = await this.#canvaAdoptDesignPage(page) || page;
      this.canvaPage = page;
      const href = String(page.url() || '');
      const inEditor = Boolean(canvaDesignId(href));
      const importing = await this.#canvaImportIndicatorVisible(page);
      if (!inEditor && Date.now() - lastNudgeAt >= 2_500) {
        lastNudgeAt = Date.now();
        await this.#canvaNudgePdfConvert(page);
      }
      const count = await this.#canvaEditorPageCount(page);
      if (inEditor && count === expectedPages && !importing) {
        await this.#waitForCanvaEditor(page, 8_000);
        this.#canvaThrowIfPageCountMismatch(count, expectedPages);
        return page;
      }
      const elapsed = Date.now() - startedAt;
      if (!inEditor && count === 0 && elapsed >= 45_000) {
        throw Object.assign(new Error(`Canva did not open the print PDF as a design after ${formatCanvaClock(elapsed)}. Still 0 of ${expectedPages} pages. Pause, use Create a design → Import file on the PDF, then try again.`), {
          code: 'CANVA_PDF_IMPORT_STUCK'
        });
      }
      if (inEditor && expectedPages > 2 && count === 1 && elapsed >= 45_000 && !importing) {
        throw Object.assign(new Error(`Canva opened a 1-page design instead of the ${expectedPages}-page PDF. Import did not convert the file. Remove the template, then import the print PDF with Create a design → Import file.`), {
          code: 'CANVA_PDF_IMPORT_BLANK_DESIGN'
        });
      }
      await tick(count, importing, href);
  ...
  async #canvaImportPdfAsDesign(page, pdfPath, expectedPages, report) {
    ...
    let openedImport = await this.#canvaClickFirst(page, [
      /^import files$/i,
      /^import file$/i,
      /import files/i,
      /import file/i
    ...
    if (!openedImport) {
      await this.#canvaClickFirst(page, [/create a design/i, /create design/i], 800);
      openedImport = await this.#canvaClickFirst(page, [
        /^import files$/i,
        /^import file$/i,
        /import files/i,
        /import file/i
    ...
    if (!openedImport) {
      await this.#canvaClickFirst(page, [/^uploads$/i, /your uploads/i, /upload files/i], 700);
    }

    await this.#pickLocalFilesInPage(page, [pdfPath], {
      clickName: /import files|import file|upload files|^upload$/i,
      mediaPicker: true
    });
    if (typeof report === 'function') {
      await report(12, `PDF selected. Opening it as a ${expectedPages}-page design…`);
    }
    await this.#canvaNudgePdfConvert(page);
    let target = await this.#canvaTakeImportTarget(page, popupPromise);
    target = await this.#canvaWaitForPdfImport(target, { expectedPages, timeoutMs: 4 * 60_000, report, percent: 12 });
  ...
  async #canvaReadTemplateLinkFromSharePanel(page) {
    await this.#canvaDismissPopups(page);
    if (!(await this.#canvaClickByName(page, /^share$/i, 2_500))) {
      throw Object.assign(new Error('Share was not found in the Canva editor.'), {
        code: 'CANVA_SHARE_MISSING'
      });
    }
    await this.#sleepOrPause(120);
    const templateVisible = await page.getByRole('button', { name: /template link/i }).first().isVisible({ timeout: 0 }).catch(() => false)
      || await page.getByText(/^template link$/i).first().isVisible({ timeout: 0 }).catch(() => false);
    if (!templateVisible) {
      await this.#canvaClickFirst(page, [/^see all$/i, /see all/i], 900);
    }
    const opened = await this.#canvaClickFirst(page, [/^template link$/i, /template link/i], 2_000);
    if (!opened) {
    ...
    await this.#canvaClickFirst(page, [/create template link/i, /create (a )?template link/i, /create (a )?link/i], 700);
    await this.#sleepOrPause(120);
    const values = await page.evaluate(() => {
      const texts = [];
      for (const node of document.querySelectorAll('input, textarea, a[href], [data-value]')) {
      ...
      for (const node of document.querySelectorAll('[role="dialog"], [aria-label*="share" i], [aria-label*="template" i]')) {
        const match = String(node.textContent || '').match(/https?:\/\/(?:www\.)?canva\.com\/[^\s"'<>]+/i);
        if (match) texts.push(match[0]);
      }
      return texts;
    }).catch(() => []);
    for (const value of values) {
      const extracted = canvaTemplateLinkFromShare(value) || String(value || '');
      if (isCanvaTemplateLink(extracted)) {
        return toCanvaTemplateLink(extracted) || extracted;
      }
    }
    for (const value of values) {
      const extracted = canvaTemplateLinkFromShare(value) || String(value || '');
      if (isCanvaDesignUrl(extracted) && !isCanvaTemplateLink(extracted)) {
  ...
  async #canvaDownloadPdf(page, downloadDir, projectId) {
    mkdirSync(downloadDir, { recursive: true });
    const destPath = join(downloadDir, `${String(projectId || 'canva').slice(0, 8)}-editable.pdf`);
    const downloadPromise = page.waitForEvent('download', { timeout: 180_000 }).catch(() => null);
    await this.#canvaClickByName(page, /^file$/i, 3_000);
    await this.#canvaClickByName(page, /^download$/i, 4_000);
    await this.#canvaClickByName(page, /pdf/i, 4_000);
    await this.#canvaClickByName(page, /download$/i, 4_000);
  ...
  async runCanvaBulkCreate({
    ...
    const report = async (percent, message, extra = {}) => {
      if (typeof onProgress !== 'function') return;
      const payload = { message, ...extra };
      if (Number.isFinite(Number(percent))) payload.percent = Number(percent);
      await onProgress(payload);
    };
    this.beginWork();
    this.canvaWatch = true;
    this.interactiveVisible = true;
    await this.#revealCanvaBrowser();
    await report(2, 'Opening Canva on screen so you can watch every click…');
    let page = await this.#canvaPage();
    await this.#revealCanvaBrowser(page);
    ...
    let resumed = false;
    const alreadyLayered = new Set((Array.isArray(layeredPageNumbers) ? layeredPageNumbers : []).map((item) => Number(item)));
    const startIndex = Math.max(0, Math.min(wantPages, Number(resumeFromIndex) || 0));

    if (savedDesignUrl) {
      await report(8, 'Continuing the Canva design already in progress…', { designUrl: savedDesignUrl });
      ...
      const editorPages = await this.#canvaEditorPageCount(page);
      ...
      } else if (editorPages > 0) {
        this.#canvaThrowIfPageCountMismatch(editorPages, wantPages);
      } else {
        page = await this.#canvaWaitForPdfImport(page, { expectedPages: wantPages, timeoutMs: 4 * 60_000, report, percent: 12 });
        resumed = true;
        designUrl = toCanvaDesignUrl(page.url()) || savedDesignUrl;
      }
    }
    ...
    if (!resumed) {
      await report(8, `Importing the finished print PDF once (${pageSize.label})…`);
    ...
    await this.#waitForCanvaEditor(page, 8_000);
    this.#canvaThrowIfPageCountMismatch(await this.#canvaEditorPageCount(page), wantPages);
    ...
    await report(20, `PDF import verified at ${wantPages} pages. Magic Layer is page by page…`, { designUrl });

    if (applyMagicLayers) {
      for (let index = 0; index < wantPages; index += 1) {
        this.#throwIfCancelled();
        const pageNumber = index + 1;
        if (alreadyLayered.size ? alreadyLayered.has(pageNumber) : index < startIndex) continue;
        const percent = 20 + Math.floor((index / wantPages) * 60);
        await this.#canvaDismissPrintReview(page);
        await report(percent, `Applying Magic Layer to page ${pageNumber} of ${wantPages}…`, {
          designUrl,
          canvaPage: { pageNumber, started: true, layered: false, error: null }
        });
        const outcome = await this.#canvaLayerOnePage(page, pageNumber, wantPages, async (_ignored, message) => {
        ...
        if (outcome.layered) {
          ...
          await report(percent + 1, `Magic Layer applied to page ${pageNumber} of ${wantPages}.`, {
            designUrl,
            canvaPage: { pageNumber, started: true, layered: true, error: null, layerCount: outcome.layerCount }
          });
        } else {
          await report(percent + 1, `Page ${pageNumber} of ${wantPages} was not separated after 3 attempts. Continuing.`, {
            designUrl,
            canvaPage: { pageNumber, started: true, layered: false, error: outcome.error }
          });
        }
      }
    }
    ...
    if (!isCanvaTemplateLink(templateLink)) {
      throw Object.assign(new Error('Share panel only showed the editor URL, not a template link. The editor URL was not saved as the buyer link.'), {
        code: 'CANVA_EDITOR_URL_NOT_TEMPLATE'
      });
    }
    designUrl = toCanvaDesignUrl(page.url()) || designUrl;
    await report(96, 'Saving the public template link…', { designUrl, templateLink });
    let exportPath = null;
    if (downloadDir) {
      exportPath = await this.#canvaDownloadPdf(page, downloadDir, projectId).catch(() => null);
    }
    await report(100, 'Canva editable layer complete.', { designUrl, templateLink });
    return {
      templateLink,
      exportPath,
      designUrl,
