...
def create_paddle_ocr(**overrides):
    ...
    try:
        ...
        raise TextInpaintError(
            "PADDLE_OCR_MISSING",
            "PaddleOCR is not installed. Install python/requirements.txt before cleaning blank masters.",
        ) from exc
    try:
...
def inpaint_lama(image, mask, inpaint_fn=None, **kwargs):
    ...
    try:
        ...
        raise TextInpaintError(
            "LAMA_REQUIRED",
            "LaMa is not installed. Install simple-lama-inpainting via python/setup_models.sh. "
