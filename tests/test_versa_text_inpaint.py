...
from versa_blank_master import (  # noqa: E402
    BASE_DILATION_PX,
    DILATION_STEP_PX,
    MAX_INPAINT_RETRIES,
    PADDLE_OCR_INIT,
...
class VisionPipelineTests(unittest.TestCase):
    def test_paddleocr_init_catches_weak_text_and_expands_unclip(self):
        ...
        source = inspect.getsource(sys.modules["versa_blank_master"].create_paddle_ocr)
...
class OptionAEngineTests(unittest.TestCase):
    def test_glyph_dilation_stays_tight(self):
        self.assertEqual(_DILATE_KERNEL, (3, 3))
        self.assertLessEqual(_DILATE_ITERATIONS, 2)

    def test_white_worksheet_uses_solid_fill(self):
        image = np.full((80, 120, 3), 255, dtype=np.uint8)
        image[22:38, 24:78] = 18
        repaired, stats = erase_text(image, [{"text": "NAME", "box": [20, 20, 80, 40]}])
        self.assertEqual(stats["engine"], "solid-fill")
        self.assertGreater(float(repaired[24:36, 30:70].mean()), 240)
