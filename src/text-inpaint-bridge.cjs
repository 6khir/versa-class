...
const PYTHON_CLI = join(__dirname, '..', 'python', 'versa_vision.py');
...
function runPython(args, { pythonBin = process.env.VERSA_PYTHON || 'python3', timeoutMs = 180_000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('error', (error) => {
      clearTimeout(timer);
      reject(Object.assign(new Error(`Python is not available for OCR/LaMa (${error.message}).`), {
